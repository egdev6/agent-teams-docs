...
function inlineMarkdown(text: string): React.ReactNode {
  ...
  while ((match = regex.exec(text)) !== null) {
    ...
    else if (match[3]) parts.push(<a key={match.index} href={match[4]} target="_blank" rel="noopener noreferrer">{match[3]}</a>);
...
function renderMarkdownBody(body: string): React.ReactNode {
    ...
    if (listItems.length > 0) {
      nodes.push(
        <ul key={`list-${key++}`} className={styles.releaseBodyList}>
          {listItems.map((item, j) => <li key={j}>{inlineMarkdown(item)}</li>)}
        </ul>
      );
      listItems = [];
    }
  ...
  for (const line of lines) {
    ...
    if (line === '' || line === '\r') {
      ...
      nodes.push(<Tag key={`h-${key++}`} className={headingClass}>{inlineMarkdown(headingMatch[2].trim())}</Tag>);
    } else if (listMatch) {
      listItems.push(listMatch[1]);
    } else {
      flushList();
      nodes.push(<p key={`p-${key++}`} className={styles.releaseBodyPara}>{inlineMarkdown(line)}</p>);
    }
  }
  flushList();
  return nodes;
...
export default function Releases() {
  const { data: releases = [], isLoading, error } = useQuery({
    queryKey: ['github-releases', GITHUB_REPO],
      ...
      <main className="container margin-vert--xl">
        ...
        {latestRelease && (
          <div className={styles.releaseNotesSection}>
            ...
            <div className={styles.latestReleaseBody}>
              {renderMarkdownBody(latestRelease.body)}
            </div>
          </div>
        )}
