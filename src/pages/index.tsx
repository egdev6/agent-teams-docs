...
function FadeInItem({
  ...
  return (
    <div
      ...
      style={{ transitionDelay: `${delay}ms`, height: '100%', ...style }}
    >
      {children}
    </div>
  );
...
const features: FeatureItem[] = [
  {
    ...
    description: translate({
      id: 'homepage.feature.models.desc',
      message:
        'Works with GitHub Copilot, Claude Code, Codex, Gemini, and any OpenAI-compatible model — no lock-in, one extension for all your agents.',
    }),
    bullets: [
  ...
  },
  {
    ...
    description: translate({
      id: 'homepage.feature.wizard.desc',
      message:
        'Four bundled AI agents guide the full setup lifecycle — auto-configure your profile, design the team, generate agent specs, and get advisory analysis — all from plain language.',
    }),
    bullets: [
      '@agent-designer & @team-builder built in',
      '@project-configurator & @consultant included',
    ],
  ...
  },
  {
    ...
    description: translate({
      id: 'homepage.feature.memory.desc',
      message:
        'Every agent — router, orchestrator, or worker — injects shared Engram memory automatically. Context survives conversations, context resets, and session changes.',
    }),
    bullets: [
      'Injected into router, orchestrator & workers',
      'Context persists across sessions & resets',
    ],
  ...
  },
  {
    ...
    description: translate({
      id: 'homepage.feature.teams.desc',
      message:
        'Bundle agents, skills, and config into a portable kit file. Drop it in any project and the full team loads instantly — no reconfiguration.',
    }),
    bullets: [
      '4 merge strategies: union, replace, append…',
      'One kit, any workspace — instantly live',
    ],
    href: '/docs/teams',
    highlight: true,
  },
  {
    ...
    description: translate({
      id: 'homepage.feature.skills.desc',
      message:
        'A plug-in instruction library for agents. Browse community skills across 9+ domains and install them in one click — no code needed.',
    }),
  ...
  },
  {
    ...
    description: translate({
      id: 'homepage.feature.context.desc',
      message:
        'Inject dynamic data into agent prompts using {{variable}} syntax, conditionals, and loops. Scoped to an agent, team, or project.',
    }),
        ...
        <FadeInItem>
          <Heading as='h2' className={styles.featuresTitle}>
            <Translate id='homepage.features.title'>
              Everything you need to build an AI agent team in VSCode
            </Translate>
          </Heading>
        </FadeInItem>
        ...
        <div className={styles.techGrid}>
          ...
          <FadeInItem delay={0}>
            <a
              ...
              href='https://github.com/Gentleman-Programming/engram?tab=readme-ov-file'
              target='_blank'
            ...
            >
              ...
              <div className={styles.techDesc}>
                <Translate id='homepage.tech.engram.desc'>
                  Persistent memory — state survives sessions and context resets
                </Translate>
              </div>
            </a>
          </FadeInItem>
          ...
          <FadeInItem delay={100}>
            ...
            >
              ...
              <div className={styles.techDesc}>
                <Translate id='homepage.tech.vscode.desc'>
                  Extension host — editors, commands, and webview panels
                </Translate>
              </div>
            </a>
          </FadeInItem>
          ...
          <FadeInItem delay={200}>
            ...
            >
              ...
              <div className={styles.techDesc}>
                <Translate id='homepage.tech.skills.desc'>
                  Plugin instruction library — SKILL.md files for instant agent specialization
                </Translate>
              </div>
            </a>
          </FadeInItem>
...
function HomepageArchitecture() {
  const mainArchitectureSteps = [
    {
      ...
      detailTitle: translate({
        id: 'homepage.arch.step1.detailTitle',
        message: 'Single entry point',
      }),
      detail: translate({
        id: 'homepage.arch.step1.detail',
        message:
          'Write @router + any task in plain language. You can reference a file, describe a vague goal, or name a domain — the pipeline handles the rest. No agent knowledge required.',
      }),
    ...
    },
    {
      ...
      desc: translate({
        id: 'homepage.arch.step2.desc',
        message: 'Scores & delegates to best agent',
      }),
      detailTitle: translate({
        id: 'homepage.arch.step2.detailTitle',
        message: 'Intent-aware dispatcher',
      }),
      detail: translate({
        id: 'homepage.arch.step2.detail',
        message:
          'Scores every agent by intent keywords, domain vocabulary, and the active file path. Delegates single-domain tasks via handoff and fans out parallel domains simultaneously — without executing anything itself.',
      }),
    ...
    },
    {
      ...
      detailTitle: translate({
        id: 'homepage.arch.step3.detailTitle',
        message: 'Planner, not executor',
      }),
      detail: translate({
        id: 'homepage.arch.step3.detail',
        message:
          'Breaks the delegated task into an ordered sequence of subtasks, assigns each to the right worker, and aggregates results. It never writes code or calls APIs directly — coordination is its only job.',
      }),
    ...
    },
    {
      ...
      desc: translate({
        id: 'homepage.arch.step4.desc',
        message: 'Domain specialists execute tasks',
      }),
      detailTitle: translate({
        id: 'homepage.arch.step4.detailTitle',
        message: 'Narrow scope, full depth',
      }),
      detail: translate({
        id: 'homepage.arch.step4.detail',
        message:
          'Each worker owns exactly one domain — backend, testing, docs, and so on. It uses only the tools and skills defined in its spec, returns a structured result, and escalates if the subtask falls outside its scope.',
      }),
      icon: <Blocks size={32} />,
    },
  ];
  ...
  const architectureExamples = [
    {
      title: translate({
        id: 'homepage.arch.examples.orchestrated.title',
        message: 'Orchestrator → Worker',
      }),
      subtitle: translate({
      ...
      steps: [
        {
          label: translate({ id: 'homepage.arch.step1.label', message: 'User Prompt' }),
          desc: translate({
            id: 'homepage.arch.step1.desc',
            message: 'Natural language via @router',
          }),
          detailTitle: translate({
            id: 'homepage.arch.examples.orchestrated.user.detailTitle',
            message: 'Complex request enters the system',
          }),
          detail: translate({
        ...
        {
          label: translate({
            id: 'homepage.arch.examples.orchestrated.orchestrator.label',
            message: 'Orchestrator',
          }),
          desc: translate({
            id: 'homepage.arch.examples.orchestrated.orchestrator.desc',
            message: 'Plans and delegates the work',
          }),
          detailTitle: translate({
            id: 'homepage.arch.examples.orchestrated.orchestrator.detailTitle',
            message: 'Planner in the middle',
          }),
          detail: translate({
        ...
        {
          label: translate({
            id: 'homepage.arch.examples.orchestrated.worker.label',
            message: 'Worker',
          }),
          desc: translate({
            id: 'homepage.arch.examples.orchestrated.worker.desc',
            message: 'Executes the delegated subtask',
          }),
          detailTitle: translate({
            id: 'homepage.arch.examples.orchestrated.worker.detailTitle',
            message: 'Specialist execution',
          }),
          detail: translate({
      ...
      steps: [
        {
          ...
          detailTitle: translate({
            id: 'homepage.arch.examples.direct.user.detailTitle',
            message: 'Small, direct request',
          }),
          detail: translate({
        ...
        },
        {
          ...
          desc: translate({
            id: 'homepage.arch.examples.direct.worker.desc',
            message: 'Handles it end-to-end',
          }),
          detailTitle: translate({
            id: 'homepage.arch.examples.direct.worker.detailTitle',
            message: 'Single specialist path',
          }),
          detail: translate({
      ...
      <div className='container'>
        ...
        <FadeInItem delay={80} style={{ height: 'auto' }}>
          <p className={styles.architectureSubtitle}>
            <Translate id='homepage.arch.subtitle'>
        ...
        <div
          ...
          style={
            { '--pipeline-step-count': String(mainArchitectureSteps.length) } as React.CSSProperties
          }
        >
              ...
              {architectureExamples.map((example, exampleIndex) => (
                <div key={example.title} className={styles.architectureExampleGroup}>
                  ...
                  <div
                    ...
                    style={
                      {
                        '--pipeline-step-count': String(example.steps.length),
                  ...
                  >
                    {example.steps.map((step, stepIndex, arr) => (
                      <React.Fragment key={`${example.title}-${step.label}`}>
                        <FadeInItem
                          delay={680 + exampleIndex * 120 + stepIndex * 80}
  ...
  return (
    <Layout
      title={translate({
        ...
        message: 'AI Agent Orchestration for VS Code',
      })}
      description={translate({
        id: 'homepage.layout.description',
        message:
          'Build, manage, and orchestrate AI agents with GitHub Copilot, Claude code or Codex inside VS Code. Create reusable agent kits, configure project profiles, and run an embedded 12-page dashboard.',
      })}
    >
      <BrowserOnly>{() => <StickyCtaBar buttonsRef={buttonsRef} />}</BrowserOnly>
      <BrowserOnly>{() => <HomepageHero heroRef={heroRef} buttonsRef={buttonsRef} />}</BrowserOnly>
      <main>
        <BrowserOnly>{() => <HomepageFeatures />}</BrowserOnly>
        <BrowserOnly>{() => <HomepagePoweredBy />}</BrowserOnly>
        <BrowserOnly>{() => <HomepageArchitecture />}</BrowserOnly>
      </main>
    </Layout>
  );
}
